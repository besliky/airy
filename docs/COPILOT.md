...
`height` inches (default 6 x 1 in at 1", 1"; every value must be a finite
number, offsets start at 0 and sizes at 0.1 in, and each is capped at
1000 in — out-of-range geometry is refused rather than saved as XML
PowerPoint would flag for repair). `apply_ops` is not available
