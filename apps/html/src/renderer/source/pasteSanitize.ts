 ...
 * pages). The html app inserts the sanitized markup straight into the source
 * buffer; in the markdown app it feeds the schema parse so Word's private
 * markup never reaches the RawHtml catch-all as junk chips.
 *
 * Deliberate twin of apps/markdown/src/renderer/editor/pasteSanitize.ts — no
 * shared package is in scope for this change, keep the two byte-equal apart
 * from this header.
 *
