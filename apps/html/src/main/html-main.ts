  ...
  ipcMain.handle(
    ...
    async (e, request: SaveHtmlRequest): Promise<SaveHtmlResult> => {
      ...
      try {
        ...
        let target = await resolveSaveTarget(e, mode, suggestedName, defaultName)
        ...
        if (pathAtRequest && resolve(pathAtRequest) === resolve(target)) {
          ...
          if (verdict !== 'fresh') {
            ...
            if (choice === 'saveAs') {
              const retry = await resolveSaveTarget(e, 'saveAs', suggestedName, defaultName)
              if (retry === 'canceled') return done({ ok: true, canceled: true })
              if (!retry) return done({ ok: false, error: 'html: no save target' })
              target = retry
            }
