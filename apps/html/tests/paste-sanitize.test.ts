...
import { sanitizeClipboardHtml } from '../src/renderer/source/pasteSanitize'

// UX-1705: clipboard HTML from Word/web is sanitized before it is inserted as
// markup into the source buffer. Byte-equal twin of the sanitizer in the
// markdown app, so this file mirrors its vectors one to one. Policy: comments
// and dangerous elements removed, everything off the allow-list unwrapped
// (inline styles CUT, not simplified), attributes stripped down to a tiny
// semantic set with http(s)/mailto-only URLs.
