...
describe('ensurePdfium under a non-ASCII process.argv[1]', () => {
  // Real WASM conversion pipeline (emscripten pdfium): duration scales with
  // machine load, and the default 20s vitest timeout is marginal under a full
  // serial suite - give it generous headroom instead of a flaky failure.
  it(
    'converts to docx and pptx when argv[1] is a CJK document path',
    { timeout: 120_000 },
    async () => {
