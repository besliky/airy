...
import { AI_CHANNELS, MARKDOWN_CHANNELS } from '../src/shared/ipc'

/**
 * Tripwire for the preload-wire contract of the markdown app (TEST-604,
 * modeled after apps/sheets/tests/preload-wire-coverage.test.ts).
 *
 * The markdown preload is a thin pass-through: it validates no payload, so
 * there is no whitelist to cross-check against a zod schema. The markdown
 * flavor of the sheets trap is channel drift against the shared channel
 * registry: MARKDOWN_/AI_CHANNELS gain an entry (or SaveMarkdownRequest loses
 * a field) while the hand-written preload keeps wiring the old surface —
 * everything compiles, the renderer just never reaches the new wire.
 *
 ...
 * - the shared MarkdownApi interface and the channel registries must still
...
// markdown:* handlers live in markdown main; app:chrome-pressed and the
// shared ai:* handlers are registered by the shell; the project:*
...
/** Frozen surface of window.markdownApi — update consciously when it grows. */
...
/** Frozen markdown channel registry — must match MARKDOWN_CHANNELS key for key. */
const PINNED_MARKDOWN_CHANNELS = `
  aiGenerateImage
  aiPanelPrefsChanged
...
/** Frozen shared-AI channel registry used by the markdown preload. */
...
describe('markdown preload wire coverage', () => {
  it('shared MarkdownApi still declares the pinned renderer surface', () => {
    const members = interfaceMembers(sharedSource, 'MarkdownApi')
    expect(members.length).toBeGreaterThan(30)
    expect([...members].sort()).toEqual(PINNED_API_MEMBERS)
  })

  it('preload implements exactly the shared MarkdownApi surface', () => {
    const members = interfaceMembers(sharedSource, 'MarkdownApi')
    const wired = apiObjectKeys(preloadSource, 'const api: MarkdownApi = {')
  ...
  it('channel registries still declare the pinned entries', () => {
    expect(Object.keys(MARKDOWN_CHANNELS).sort()).toEqual(PINNED_MARKDOWN_CHANNELS)
    expect(Object.keys(AI_CHANNELS).sort()).toEqual(PINNED_AI_CHANNELS)
  })

  it('preload wires every entry of both channel registries', () => {
    // A registry entry the preload never references is the markdown flavor
    // of the sheets trap: the shared contract grew, the wire did not.
    const unwired = [
      ...Object.keys(MARKDOWN_CHANNELS)
        .filter((key) => !new RegExp(`MARKDOWN_CHANNELS\\.${key}\\b`).test(preloadSource))
        .map((key) => `MARKDOWN_CHANNELS.${key}`),
  ...
  it('pins the SaveMarkdownRequest wire fields', () => {
    const fields = interfaceMembers(sharedSource, 'SaveMarkdownRequest')
