 ...
 * program (or another window on the same file) wrote it since we last read/saved
 * it. No record (path never tracked) or a missing file (deleted externally) is
 * not a conflict — the save proceeds and recreates the file. The hash read only
 * runs when mtime+size already disagree, so the common no-conflict save never
 * rereads the file. (Docs #151 fence, ported for in-place PDF saves.)
 */
