...
import type { AiChatResponse, AiSettings, AiStreamChunk } from '@airy-office/ai-provider'
...
// Off by default. e2e drivers launch the BUILT app with AIRY_DEBUG_HOOKS=1
// so the renderer exposes window.__airyDebug (see App.tsx) — the dev-only
// __univerAPI hook does not exist in production bundles.
if (process.env.AIRY_DEBUG_HOOKS === '1') {
  contextBridge.exposeInMainWorld('__airyDebugHooks', true)
}
