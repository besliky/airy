...
async function createStandaloneSheetsDocument(
  ...
  if (request.type === 'docx') {
    return { ok: false, error: 'Creating DOCX files requires the Airy shell or Docs app.' }
  }
...
async function snapshotWorkbook(path: string): Promise<string> {
  const dir = join(app.getPath('temp'), 'airy-sheets-sessions')
...
async function prepareWorkbookForOpen(
  ...
  const directory = join(app.getPath('temp'), 'airy-imports', randomUUID())
