...
describe('InMemoryWorkbookAdapter', () => {
  ...
  it('deletes a sheet, warning about formulas that reference it', () => {
    ...
    // Excel semantics (BUG-1662): the dependent formula rewrote to #REF!
    // instead of silently keeping a dangling reference.
    expect(adapter.getSnapshot().sheets[0]?.cells.B1?.formula).toBe('=#REF!*2')
    ...
    // Snapshot-history undo restores the sheet AND the original formula.
    adapter.undo()
    expect(adapter.getSnapshot().sheets.map((sheet) => sheet.id)).toEqual(['sheet-1', 'sheet-2'])
    expect(adapter.getSnapshot().sheets[1]?.cells.B1?.formula).toBe('=Data!A1*2')
  })

  it('keeps formulas without references to the deleted sheet untouched', () => {
    const adapter = new InMemoryWorkbookAdapter({
      revision: 0,
      sheets: [
        { id: 'sheet-1', name: 'Data', cells: { A1: { value: 1 } } },
        {
          id: 'sheet-2',
          name: 'Summary',
          cells: { B1: { value: null, formula: '=SUM(A1:A2)' } },
        },
      ],
    })
    adapter.apply(
      adapter.plan({
        dslVersion: 1,
        transactionId: 'tx-del-sheet-clean',
        baseRevision: 0,
        summary: 'Delete the Data sheet',
        operations: [{ op: 'delete_sheet', sheetId: 'sheet-1' }],
      }),
    )
    expect(adapter.getSnapshot().sheets[0]?.cells.B1?.formula).toBe('=SUM(A1:A2)')
  })
