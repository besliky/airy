 ...
 * `.destroy()` — a tracking collection nobody drains (the cross-ref-modal
 * regression) is a leak even though every site looks registered.
 *
...
/** every `new Editor(`/`new EditorView(` site that fails the traits above */
const scan = (): Discipline => {
  ...
  for (const file of listTestFiles(TESTS)) {
    ...
    const hasDestroy = /\.destroy\(\)/.test(src)
    if (!hasDestroy)
      violations.push(`${rel}: mounts ${sites.length} editor(s) but has no .destroy() anywhere`)
