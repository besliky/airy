...
/**
 * UX-1206 dropdown naming contract (docs twin of the slides rule in
 ...
 * label — which overrides any visible field label or ScreenTip it sits
 * next to. Without an explicit `ariaLabel` the control answers to its
 * current VALUE («1 pt», «In Line with Text»), not its field.
 *
 ...
 * audit's finding was slides-side; the docs ribbon dropdowns below are the
 * same class, registered so the number can only shrink). A new dropdown
 * MUST carry an ariaLabel to stay at zero in unlisted files.
 */
...
const ALLOWANCE: Record<string, number> = {
  // picture border weight, wrap text, table border width
  'components/Ribbon.tsx': 3,
}
