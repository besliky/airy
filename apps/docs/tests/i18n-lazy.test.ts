...
// PERF-904: dictionaries load per locale on demand. These tests pin the lazy
...
describe('loadStrings (lazy per-locale dictionaries)', () => {
  it('resolves a dictionary covering the zh key set across all eight domains', async () => {
    const en = await loadStrings('en')
    expect(en.appScTitle).toBe('Keyboard Shortcuts')
    expect(en.editorFootnote).toBe('Footnote')
    // a table-domain key proves the per-locale merge module stitched all shards
...
describe('module translator across a language switch', () => {
  it('falls back to the raw key while no dictionary is loaded', () => {
    setModuleLang('hi')
    expect(t('editorFootnote')).toBe('editorFootnote')
  })

  it('translates with the loaded locale and follows a switch', async () => {
    await loadLocale('en')
    setModuleLang('en')
    expect(t('editorFootnote')).toBe('Footnote')

    await loadLocale('ru')
    setModuleLang('ru')
    expect(t('editorFootnote')).toBe('Сноска')
  ...
  it('fills {placeholder} params like the eager table did', async () => {
    await loadLocale('en')
    setModuleLang('en')
    expect(t('editorIndexEntry', { term: 'photosynthesis' })).toBe('Index entry: photosynthesis')
  })
})
