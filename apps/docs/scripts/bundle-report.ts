/// Renderer bundle size report for docs, modeled on the sheets report
/// (PERF-502/903). Reads apps/docs/out/renderer after `electron-vite build`
/// and prints the chunk table: which chunks load eagerly at startup (the
/// entry script plus everything index.html modulepreloads), which stay lazy
/// (the PERF-904 locale chunks), and whether budgets hold. Two budgets: any
/// single chunk over 5 MB (the shared PERF-502 ceiling) and the entry chunk
/// over 2,200 kB — the docs-specific gate with headroom over the ~2.0 MB
/// entry PERF-904 landed. Exits non-zero on a violation so CI can gate the
/// build before e2e minutes are spent.
///
/// Usage: npm run bundle-report [-w @airy-office/docs] [--json]
import { readdir, readFile, stat } from 'node:fs/promises'
import { dirname, join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'

/// Largest allowed single chunk (bytes) — the same PERF-502 ceiling the
/// sheets report enforces.
const CHUNK_BUDGET_BYTES = 5 * 1024 * 1024

/// Largest allowed entry chunk (bytes) — PERF-904 brought the entry from
/// ~3.45 MB down to ~2.0 MB; 2,200 kB keeps ~10% headroom over that
/// landing spot while still catching a regression toward the monolith.
const ENTRY_BUDGET_BYTES = 2_200 * 1024
