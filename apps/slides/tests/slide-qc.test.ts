...
import type { AgentStreamRequest, AgentTransport } from '@airy-office/agent-core'
import {
  generatedPageRange,
  mergeQcPages,
  createSlideFixSkill,
...
import { defaultAiSettings, type AiProviderId } from '@airy-office/ai-provider'
...
describe('vision capability fallback', () => {
  ...
  it('does not send screenshots to text-only models under a vision-capable provider', () => {
    ...
    settings.providers.openai.model = 'deep-seek-v4-flash'
    expect(settingsSupportVision(settings)).toBe(false)
    settings.providers.openai.model = 'gpt-5.6-sol'
