...
describe('regenerate_slide', () => {
  it('brief → page generation produces the marker → calls access.regenerateSlide to land it', async () => {
  ...
  it('page generation fails (after 1 retry) → error passed through', async () => {
    const generatePageLocal = vi.fn(async () => ({ ok: false, error: 'generation timeout' }))
    const skill = createSlidesSkill(
      mkAccess([page], {
        regenerateSlide: vi.fn(async () => ({ ok: true })),
        generatePageLocal,
    ...
    expect(r.isError).toBe(true)
    expect(r.output).toContain('generation timeout')
    expect(generatePageLocal).toHaveBeenCalledTimes(2)
  })

  it('the local spec builder produces the marker and lands it', async () => {
    const regenerateSlide = vi.fn(async () => ({ ok: true }))
