...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'airy-slides-temp-test-'))
  roots.push(root)
  return root
}
...
describe('generated-page temp sweep', () => {
  it('removes only expired .pptx files matching the UUID pattern', async () => {
    ...
    const cloudDir = join(root, 'genoffice-cloud-pages') // legacy name still swept
    const localDir = join(root, 'airy-local-pages')
  ...
  it('skips directories and non-.pptx files', async () => {
    ...
    const dirLike = join(root, 'airy-cloud-pages', 'not-a-file')
