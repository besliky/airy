...
import { createBlankPptx, openPptx } from '@airy-office/pptx-engine'
...
beforeEach(() => {
  availability.clear()
  vi.stubEnv('AIRY_FONT_CDN_URL', fontCdnBaseUrl)
...
describe('font catalog', () => {
  ...
  it('extracts and validates the packaged CDN URL', () => {
    expect(
      extractFontCdnBaseUrl({
        airyFontCdn: { baseUrl: ' https://fonts.example.test/v1/ ' },
      }),
    ).toBe(fontCdnBaseUrl)
    expect(
      extractFontCdnBaseUrl({ airyFontCdn: { baseUrl: 'http://fonts.example.test/v1' } }),
    ).toBeNull()
    expect(
      extractFontCdnBaseUrl({
        airyFontCdn: { baseUrl: 'https://user@fonts.example.test/v1' },
      }),
    ).toBeNull()
    expect(
      extractFontCdnBaseUrl({
        airyFontCdn: { baseUrl: 'https://fonts.example.test/v1?token=secret' },
      }),
    ).toBeNull()
  })
})
