 ...
 *
 * A curated catalog of OFL-licensed families is mirrored on the font CDN
...
import type { OpenedPptx } from '@airy-office/pptx-engine'
...
export function extractFontCdnBaseUrl(pkg: unknown): string | null {
  ...
  const raw = (pkg as Record<string, unknown>).airyFontCdn
...
export function fontCdnBaseUrl(): string | null {
  if (!app.isPackaged) return normalizeFontCdnBaseUrl(process.env.AIRY_FONT_CDN_URL)
  try {
