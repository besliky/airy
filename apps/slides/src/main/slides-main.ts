/**
 * Airy Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/Airy):
...
/** Directory where AI-generated drafts are saved: the configurable default save folder (falls back to <Documents>/Airy) */
...
export function registerSlidesIpc(): void {
  ...
  // ── Local single-page generation (e.g. BYOK): a JSON slide spec written by
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const dir = join(app.getPath('temp'), 'airy-local-pages')
  ...
  ipcMain.handle('slides:copy-slide', (e, slideIndex: number, pngBase64?: string) => {
    ...
    clipboard.writeBuffer('io.airy.slides.slide', Buffer.from('1'))
    return true
  })
  ...
  ipcMain.handle('slides:clipboard-external', () => {
    if (slideClipboard && clipboardMarker('io.airy.slides.slide')) return { kind: 'slide' }
    if (elementClipboard && clipboardMarker('io.airy.slides.elements')) return { kind: 'internal' }
  ...
  ipcMain.handle('slides:clipboard-probe', () => {
    if (slideClipboard && clipboardMarker('io.airy.slides.slide')) return true
    if (elementClipboard && clipboardMarker('io.airy.slides.elements')) return true
  ...
  ipcMain.handle('slides:copy-elements', (e, op: CopyElementsOp) => {
    ...
    if (items.length) {
      ...
      clipboard.writeBuffer('io.airy.slides.elements', Buffer.from('1'))
    }
    return items.length
  })
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    await app.whenReady()
    // PAC/rule proxies answer per-host: probe a host the LLM APIs live behind
...
export function startSlidesStandalone(): void {
  ...
  // AIRY_USER_DATA: test drivers point this at a scratch dir so automated
  ...
  if (!app.isPackaged && process.env.AIRY_USER_DATA) {
    app.setPath('userData', process.env.AIRY_USER_DATA)
  }
