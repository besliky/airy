 ...
 * place that reads ai-settings.json live and routes to the BYOK media
 * provider when one is configured. BYOK providers answer with bytes; those
 * land in the local generated-image store and come back as a file:// URL
 * that the insert pipelines' fetchRemoteImage accepts.
 */
...
export const ANALYSIS_PROVIDER_NOT_CONFIGURED_ERROR =
  'No media analysis provider is configured; ask the user to configure one under Settings (AI Media) to use this tool'

/** 200 MB: enough for a long clip through the Gemini Files API, small enough to hold in memory */
const MAX_MEDIA_BYTES = 200 * 1024 * 1024

/** size-cap failure, recognized by callers that stream large media themselves */
...
export async function analyzeMediaTool(
  ...
  try {
    if (!imageByok && !videoByok) return { error: ANALYSIS_PROVIDER_NOT_CONFIGURED_ERROR }
    ...
    if (!byok) return { error: ANALYSIS_PROVIDER_NOT_CONFIGURED_ERROR }
    return {
