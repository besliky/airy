...
import type { AgentToolCall } from '@airy-office/agent-core'
...
describe('streamForProvider: request headers', () => {
...
describe('streamForProvider: 200 + non-stream JSON instead of SSE', () => {
  const creditsNotice =
    'Your credits have been exhausted. Please visit the pricing page to purchase more credits.'
  ...
  it('gemini route: zero usage + a credits-worded notice counts as a credits notice', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
          candidates: [
            {
              content: {
                parts: [{ text: 'Out of quota: insufficient credits remaining.' }],
              },
            },
          ],
...
describe('streamForProvider: a connection dropped mid tool arguments is not an empty stream', () => {
  // Tool arguments are buffered upstream; a gateway can close the SSE
