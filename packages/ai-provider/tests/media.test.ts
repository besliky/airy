...
describe('media settings', () => {
  it('defaults every provider to its default models with neutral BYOK picks and no key', () => {
    const media = defaultAiMediaSettings()
  ...
  it('is carried by defaultAiSettings and healed in from a pre-media settings file', () => {
    ...
    const resolved = resolveAiSettings(
      { provider: 'kimi', providers: defaults.providers },
      defaultAiSettings(),
    )
  ...
  it('activates a BYOK media provider per capability, only when usable and capable', () => {
    ...
    // MiniMax has no analysis endpoint: picking it for analysis deactivates it
    const mm = defaultAiMediaSettings()
    mm.analysisProvider = 'minimax'
    mm.providers.minimax.apiKey = 'k'
    expect(activeMediaProvider(withMedia(mm), 'analysis')).toBeNull()
    // OpenAI reads images but not video: as the video provider it deactivates
  ...
  it('gates the tools on a usable BYOK model alone', () => {
    // fresh defaults carry no key: nothing is available yet
    expect(imageGenerationAvailable(defaultAiSettings())).toBe(false)
