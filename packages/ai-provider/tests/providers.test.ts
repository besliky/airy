...
describe('provider model catalog', () => {
  ...
  it('no longer carries a genspark entry (retired provider)', () => {
    expect(
      AI_PROVIDERS.find((provider) => (provider as { id: string }).id === 'genspark'),
    ).toBeUndefined()
    expect(AI_PROVIDERS.find((provider) => provider.id === 'none')).toBeDefined()
  })
...
describe('activeProvider', () => {
  it('honors a configured BYOK provider and falls back to none otherwise', () => {
    const settings = defaultAiSettings()
    expect(activeProvider(settings)).toBe('none')

    settings.provider = 'kimi'
    expect(activeProvider(settings)).toBe('none') // no key yet
  ...
  it('requires a base URL for providers that declare needsBaseUrl', () => {
    ...
    expect(activeProvider(settings)).toBe('none') // custom's default model is empty
  ...
  it('falls back to none for unknown or retired ids from a hand-edited settings file', () => {
    const settings = defaultAiSettings()
    settings.provider = 'nonsense' as AiProviderId
    expect(activeProvider(settings)).toBe('none')
    // a pre-fork file still selecting the retired genspark provider reads as unconfigured
