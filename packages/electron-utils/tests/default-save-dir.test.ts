...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'airy-save-dir-'))
})
...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'Airy')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/Airy without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'Airy'))
    expect(existsSync(join(documents, 'Airy'))).toBe(true)
  })
})
