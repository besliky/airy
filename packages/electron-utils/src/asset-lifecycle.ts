...
/**
 * App-owned image asset lifecycle, shared verbatim by the markdown and html
 * editors (sibling documents may reference the same `assets/` directory, so
 * garbage collection must scan them — only the file extensions differ).
 */
export interface AssetLifecycleScope {
  /** Lowercase app label used in thrown error messages ('markdown' / 'html'). */
  appLabel: string
  /** Which sibling documents may reference owned assets, e.g. /\.(?:md|markdown)$/i. */
  siblingDocumentPattern: RegExp
}
...
export interface OwnedAssetManifest {
  ...
  /** Document file name -> app-owned files referenced by its last successful save. */
...
async function existingAssetsDir(
  scope: AssetLifecycleScope,
  documentPath: string,
): Promise<string | null> {
  ...
  try {
    ...
    if (!info.isDirectory() || info.isSymbolicLink()) {
      throw new Error(`${scope.appLabel} assets path must be a real directory`)
    }
    return assetsDir
  } catch (error) {
...
async function ensureAssetsDir(scope: AssetLifecycleScope, documentPath: string): Promise<string> {
  const existing = await existingAssetsDir(scope, documentPath)
...
async function readManifestUnlocked(
  scope: AssetLifecycleScope,
  documentPath: string,
): Promise<OwnedAssetManifest> {
  const assetsDir = await existingAssetsDir(scope, documentPath)
...
async function writeManifestUnlocked(
  ...
  const assetsDir = await ensureAssetsDir(scope, documentPath)
...
async function addOwnedRecord(
  scope: AssetLifecycleScope,
  documentPath: string,
  record: OwnedAssetRecord,
): Promise<void> {
  await withManifestLock(documentPath, async () => {
    const manifest = await readManifestUnlocked(scope, documentPath)
    ...
    await writeManifestUnlocked(scope, documentPath, manifest)
  })
}
...
async function createOwnedAsset(
  ...
  for (let suffix = 0; suffix < 100_000; suffix++) {
    ...
    try {
      await addOwnedRecord(scope, documentPath, { name, sha256: await sha256File(target) })
      return `assets/${name}`
    } catch (error) {
  ...
  throw new Error(`${scope.appLabel} asset name space exhausted`)
}

async function copyImageIntoOwnedAssets(
  scope: AssetLifecycleScope,
  documentPath: string,
  sourcePath: string,
): Promise<string> {
  return createOwnedAsset(scope, documentPath, basename(sourcePath), (target) =>
    copyFile(sourcePath, target, constants.COPYFILE_EXCL),
  )
}

async function writeImageIntoOwnedAssets(
  scope: AssetLifecycleScope,
  documentPath: string,
  preferredName: string,
  bytes: Buffer,
): Promise<string> {
  return createOwnedAsset(scope, documentPath, preferredName, (target) =>
...
async function siblingDocumentReferences(
  scope: AssetLifecycleScope,
  documentPath: string,
  ...
  try {
    for (const entry of await readdir(documentDir, { withFileTypes: true })) {
      if (
        ...
        !scope.siblingDocumentPattern.test(entry.name)
      ) {
        continue
      }
...
/** Image sources in document text (markdown `![]()` and HTML `<img src>` alike). */
export function extractImageSources(text: string): string[] {
  return scanImageSources(text).ranges.map((range) => range.source)
}
...
/** Replace image sources in document text, HTML-attribute-encoding when needed. */
export function rewriteImageSources(text: string, rewrites: ReadonlyMap<string, string>): string {
  if (rewrites.size === 0) return text
  const ranges = scanImageSources(text).ranges
  let cursor = 0
  let output = ''
...
async function prepareAssetsForSaveAs(
  scope: AssetLifecycleScope,
  ...
  for (const source of extractImageSources(text)) {
  ...
  const targetAssetsDir = await ensureAssetsDir(scope, targetDocumentPath)
  try {
    await withManifestLock(targetDocumentPath, async () => {
      const manifest = await readManifestUnlocked(scope, targetDocumentPath)
      ...
      for (const source of allImageSources) {
        ...
        if (!copied) throw new Error(`${scope.appLabel} asset name space exhausted`)
        created.push(copied)
      ...
      const rewrittenDestinations = new Set(
        extractImageSources(rewriteImageSources(text, rewrites)),
      )
      ...
      await writeManifestUnlocked(scope, targetDocumentPath, manifest)
    })
  } catch (error) {
    for (const record of created) {
  ...
  return {
    targetDocumentPath,
    text: rewriteImageSources(text, rewrites),
...
async function rollbackPreparedSaveAsAssets(
  scope: AssetLifecycleScope,
  plan: PreparedSaveAsAssets,
): Promise<void> {
  await removeCreatedRecords(scope, plan.targetDocumentPath, plan.created)
}

async function pendingOwnedAssetsForDocument(
  scope: AssetLifecycleScope,
  documentPath: string,
): Promise<string[]> {
  try {
...
async function resolvePendingOwnedAssets(
  ...
  try {
    return await withManifestLock(documentPath, async () => {
      ...
      for (const name of await siblingDocumentReferences(scope, documentPath, ownedNames, true)) {
        referenced.add(name)
      }
 ...
 */
async function discardPendingOwnedAssets(
  scope: AssetLifecycleScope,
  documentPath: string,
): Promise<ReconcileResult> {
  return resolvePendingOwnedAssets(scope, documentPath)
}
 ...
 */
async function resolveSourcePendingAfterSaveAs(
  scope: AssetLifecycleScope,
  sourceDocumentPath: string,
  pendingAtSaveStart: readonly string[],
): Promise<ReconcileResult> {
  return resolvePendingOwnedAssets(scope, sourceDocumentPath, pendingAtSaveStart)
}

async function reconcileOwnedAssets(
  scope: AssetLifecycleScope,
  documentPath: string,
  imageSources: readonly string[],
  ...
  try {
    return await withManifestLock(documentPath, async () => {
      ...
      for (const name of await siblingDocumentReferences(scope, documentPath, ownedNames)) {
        referenced.add(name)
      }
...
async function renameOwnedAssetDocument(
  scope: AssetLifecycleScope,
  ...
  await withManifestLock(newDocumentPath, async () => {
    const manifest = await readManifestUnlocked(scope, newDocumentPath)
    ...
    await writeManifestUnlocked(scope, newDocumentPath, manifest)
  })
}

async function readOwnedAssetManifest(
  scope: AssetLifecycleScope,
  documentPath: string,
  ...
  try {
    ...
    return await readManifestUnlocked(scope, documentPath)
  } catch {
    return null
  }
}

/** The app-parameterized asset lifecycle API (see createAssetLifecycle). */
export interface AssetLifecycle {
  copyImageIntoOwnedAssets(documentPath: string, sourcePath: string): Promise<string>
  ...
  rollbackPreparedSaveAsAssets(plan: PreparedSaveAsAssets): Promise<void>
  pendingOwnedAssetsForDocument(documentPath: string): Promise<string[]>
  discardPendingOwnedAssets(documentPath: string): Promise<ReconcileResult>
  ...
  renameOwnedAssetDocument(oldDocumentPath: string, newDocumentPath: string): Promise<void>
  readOwnedAssetManifest(documentPath: string): Promise<OwnedAssetManifest | null>
}

/**
 * Bind the shared asset lifecycle to one editor app. The scope carries the
 * only two app-specific behaviors: the app label for error messages and which
 * sibling document extensions may reference the shared assets/ directory.
 */
export function createAssetLifecycle(scope: AssetLifecycleScope): AssetLifecycle {
  return {
    copyImageIntoOwnedAssets: (documentPath, sourcePath) =>
      copyImageIntoOwnedAssets(scope, documentPath, sourcePath),
    writeImageIntoOwnedAssets: (documentPath, preferredName, bytes) =>
      writeImageIntoOwnedAssets(scope, documentPath, preferredName, bytes),
    prepareAssetsForSaveAs: (sourceDocumentPath, targetDocumentPath, text, imageSources) =>
      prepareAssetsForSaveAs(scope, sourceDocumentPath, targetDocumentPath, text, imageSources),
    rollbackPreparedSaveAsAssets: (plan) => rollbackPreparedSaveAsAssets(scope, plan),
    pendingOwnedAssetsForDocument: (documentPath) =>
      pendingOwnedAssetsForDocument(scope, documentPath),
    discardPendingOwnedAssets: (documentPath) => discardPendingOwnedAssets(scope, documentPath),
    resolveSourcePendingAfterSaveAs: (sourceDocumentPath, pendingAtSaveStart) =>
      resolveSourcePendingAfterSaveAs(scope, sourceDocumentPath, pendingAtSaveStart),
    reconcileOwnedAssets: (documentPath, imageSources, options) =>
      reconcileOwnedAssets(scope, documentPath, imageSources, options),
    renameOwnedAssetDocument: (oldDocumentPath, newDocumentPath) =>
      renameOwnedAssetDocument(scope, oldDocumentPath, newDocumentPath),
    readOwnedAssetManifest: (documentPath) => readOwnedAssetManifest(scope, documentPath),
  }
}
