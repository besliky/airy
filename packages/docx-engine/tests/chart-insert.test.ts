...
describe('saveDocx kind:chart — new kinds round-trip', () => {
  ...
  it('workbook sync merges the data rectangle: formulas and outside cells survive (BUG-1008)', async () => {
    ...
    // outside the rectangle: row 5 and its formulas/notes byte-identical
    expect(out).toContain(
      '<row r="5"><c r="D5"><f>SUM(B2:B3)</f><v>3</v></c>' +
        '<c r="E5" t="inlineStr"><is><t>note</t></is></c></row>',
    )
