 ...
 * userData dir (via AIRY_USER_DATA) so runs never touch real settings
...
interface LaunchOptions {
  ...
  userDataDir?: string
  /** UI language override (AIRY_LANG); defaults to English for stable assertions */
  lang?: string
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  const userDataDir = options.userDataDir ?? (await mkdtemp(join(tmpdir(), 'airy-e2e-')))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...hostEnv,
      AIRY_USER_DATA: userDataDir,
      AIRY_LANG: options.lang ?? 'en',
